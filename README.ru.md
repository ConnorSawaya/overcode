...
brew install overcode # macOS и Linux (рекомендуем, всегда актуально)
brew install overcode              # macOS и Linux (официальная формула brew, обновляется реже)
sudo pacman -S overcode            # Arch Linux (Stable)
paru -S overcode-bin               # Arch Linux (Latest from AUR)
mise use -g overcode               # любая ОС
nix run nixpkgs#overcode           # или github:ConnorSawaya/overcode для самой свежей ветки dev
```
...
Overcode также доступен как десктопное приложение. Скачайте его со [страницы релизов](https://github.com/ConnorSawaya/overcode/releases) или с [overcode.ai/download](https://github.com/ConnorSawaya/overcode/releases).
...
1. `$OVERCODE_INSTALL_DIR` - Пользовательский каталог установки
2. `$XDG_BIN_DIR` - Путь, совместимый со спецификацией XDG Base Directory
3. `$HOME/bin` - Стандартный каталог пользовательских бинарников (если существует или можно создать)
4. `$HOME/.overcode/bin` - Fallback по умолчанию
...
В Overcode есть два встроенных агента, между которыми можно переключаться клавишей `Tab`.
...
Если вы хотите внести вклад в Overcode, прочитайте [contributing docs](./CONTRIBUTING.md) перед тем, как отправлять pull request.

### Разработка на базе Overcode

Если вы делаете проект, связанный с Overcode, и используете "overcode" как часть имени (например, "overcode-dashboard" или "overcode-mobile"), добавьте примечание в README, чтобы уточнить, что проект не создан командой Overcode и не аффилирован с нами.

---

**Присоединяйтесь к нашему сообществу** [Discord](https://discord.gg/overcode) | [X.com](https://x.com/overcode)
