...
brew install overcode # macOS 和 Linux（推荐，始终保持最新）
brew install overcode              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S overcode            # Arch Linux (Stable)
paru -S overcode-bin               # Arch Linux (Latest from AUR)
mise use -g overcode               # 任意系统
nix run nixpkgs#overcode           # 或用 github:ConnorSawaya/overcode 获取最新 dev 分支
```
...
Overcode 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/ConnorSawaya/overcode/releases) 或 [overcode.ai/download](https://github.com/ConnorSawaya/overcode/releases) 下载。
...
1. `$OVERCODE_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.overcode/bin` - 默认备用路径
...
Overcode 内置两种 Agent，可用 `Tab` 键快速切换：
...
如果你在项目名中使用了 “overcode”（如 “overcode-dashboard” 或 “overcode-mobile”），请在 README 里注明该项目不是 Overcode 团队官方开发，且不存在隶属关系。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/overcode)
