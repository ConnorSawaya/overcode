...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ overcode }) =>
    Effect.gen(function* () {
      const result = yield* overcode.spawn(["--replay-limit", "10"])

      overcode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ overcode }) =>
    Effect.gen(function* () {
      const result = yield* overcode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ overcode }) =>
    Effect.gen(function* () {
      const result = yield* overcode.spawn(["--mini", "--port", "4096"])
