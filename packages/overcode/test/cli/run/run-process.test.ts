// Subprocess integration tests for `overcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `overcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OVERCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("overcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* overcode.run("say hi")
        overcode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* overcode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* overcode.run("think", { extraArgs: ["--thinking"] })
        overcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* overcode.run("think again")
        overcode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* overcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* overcode.run("say hi", { format: "json" })
        overcode.expectExit(result, 0)

        const events = overcode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ overcode }) =>
      Effect.gen(function* () {
        const result = yield* overcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* overcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* overcode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* overcode.run("request permission", { permission: { bash: "ask" } })
        overcode.expectExit(denied, 0)
        ...
        const allowed = yield* overcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        overcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* overcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        overcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, overcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* overcode.serve()

        const result = yield* overcode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, overcode }) =>
      Effect.gen(function* () {
        const result = yield* overcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, overcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* overcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
