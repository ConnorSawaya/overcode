...
describe("overcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OVERCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["mcp", "list"])
        overcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["providers", "list"])
        overcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["models"])
        overcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["agent", "list"])
        overcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OVERCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["session", "list"])
        overcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["stats"])
        overcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OVERCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ overcode }) =>
      Effect.gen(function* () {
        const r = yield* overcode.spawn(["db", "path"])
        overcode.expectExit(r, 0, "db path")
