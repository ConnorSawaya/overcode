...
describe("overcode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ overcode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ overcode }))
        ...
        expect(initialized.agentInfo?.name).toBe("Overcode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ overcode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ overcode })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OVERCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
