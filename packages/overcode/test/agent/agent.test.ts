...
it.instance("plan agent denies edits except .overcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".overcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".overcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OVERCODE_TEST_HOME
      process.env.OVERCODE_TEST_HOME = test.directory
