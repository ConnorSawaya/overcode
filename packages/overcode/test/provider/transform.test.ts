    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".overcode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, overcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.overcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.overcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              overcode: { itemId: "msg_overcode_part" },
    ...
    expect(result[0].providerOptions?.overcode?.itemId).toBe("msg_overcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.overcode?.itemId).toBe("msg_overcode_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["overcode", "deepseek-v4-flash"],
    ["overcode-go", "deepseek-v4-flash"],
