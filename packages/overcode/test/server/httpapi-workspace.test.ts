...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-overcode-directory", directory)
  ...
  it.live("proxies remote workspace HTTP requests with sanitized forwarding", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-overcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-overcode-workspace")
