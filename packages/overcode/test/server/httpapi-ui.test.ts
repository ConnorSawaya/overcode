  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("OVERCODE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("OVERCODE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
...
function authConfigLayer(input?: { password?: string; username?: string }) {
  return ServerAuth.Config.configLayer({
    ...
    username: input?.username ?? "overcode",
  })
}
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            OVERCODE_SERVER_PASSWORD: input?.password,
            OVERCODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>overcode</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>overcode</html>")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>overcode</html>", {
                      headers: {
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "overcode",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>overcode</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("overcode:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("overcode:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("overcode:sec:ret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "overcode" }).request("/", {
        method: "OPTIONS",
        headers: {
