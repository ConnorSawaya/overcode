...
Delete `packages/overcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/overcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/overcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/overcode`
- TUI plugin loader tests in `packages/overcode` if runtime plugin API wiring changed
