...
Practical reference for a future `packages/server` split after the overcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/overcode`.
...
Until enough shared service code lives outside `packages/overcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/overcode`

It should not import `packages/overcode` services while `packages/overcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on overcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/overcode` implementation details.
