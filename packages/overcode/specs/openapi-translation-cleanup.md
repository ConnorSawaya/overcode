...
Trim `packages/overcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/overcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/overcode`.
- `[x]` `bun typecheck` from `packages/overcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/overcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/overcode`.
- `[x]` `bun dev generate > /tmp/overcode-openapi.json` from `packages/overcode`.
...
- Edit `packages/overcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/overcode-openapi.json` from `packages/overcode`.
...
- Check source schemas in files like `packages/overcode/src/session/schema.ts`, `packages/overcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/overcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/overcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
