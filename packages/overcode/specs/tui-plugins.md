...
```json
{
  ...
  "plugin": ["@acme/overcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- The value is a semver range checked against the running Overcode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.overcode` is absent, no check is performed (backward compatible).
...
- `overcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `overcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.overcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.overcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.overcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.overcode` dir or `OVERCODE_CONFIG_DIR`, Overcode installs `@opencode-ai/plugin` into that dir and writes:
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Overcode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Overcode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Overcode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- `message` is required; `title` defaults to `"overcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `overcode.default`.
...
- Local plugins persist installed themes under the local `.overcode/themes` area near the plugin config source.
...
- `--pure` / `OVERCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.overcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.overcode/plugins/tui-vim.tsx`
- Local smoke config: `.overcode/tui.json`
- Local smoke theme: `.overcode/plugins/smoke-theme.json`
