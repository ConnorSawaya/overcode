...
export const BrowserTool = Tool.define(
  "browser",
  Effect.succeed({
    description: `Control the real session-owned Chromium browser in the Overcode desktop app. Use this for testing local web apps, inspecting pages, and interacting with forms. Browser actions are observable in the user's browser panel and use semantic targets where possible.
...
Targets may be role=button name="Continue", label=Email, placeholder=Search, text=Settings, css=#submit, or a concise visible name. Browser screenshot returns an image attachment; coordinates for browser move use the reported viewport size.`,
    ...
    execute: (input: Input, ctx: Tool.Context) =>
      Effect.gen(function* () {
        const bridge = process.env.OVERCODE_BROWSER_BRIDGE_URL
        const token = process.env.OVERCODE_BROWSER_BRIDGE_TOKEN
        if (!bridge || !token) {
          return {
            title: "Browser unavailable",
            output: "The session-owned browser is available in the Overcode desktop build, but this server is not connected to the desktop browser bridge.",
          ...
          try: async () => {
            const result = await fetch(`${bridge}/action`, {
              ...
              headers: {
                ...
                "x-overcode-browser-token": token,
              },
              body: JSON.stringify({
                ...input,
