...
const AGENT_USERNAME = "overcode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/overcode.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/overcode-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.overcode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  overcode:
    if: |
      ...
      startsWith(github.event.comment.body, '/overcode')
    ...
    steps:
      ...
      - name: Run overcode
        uses: ConnorSawaya/overcode/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.overcode.ai" : "https://github.com/ConnorSawaya/overcode"
    ...
    try {
      ...
      console.log("overcode session", session.id)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.overcode.ai"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/overcode,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to overcode...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("overcode-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `overcode/${type}-${hex}-${timestamp}`
      }
      return `overcode/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/overcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[overcode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the overcode infrastructure after your response",
