...
export class Service extends Context.Service<Service, Interface>()("@overcode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["overcode.jsonc", "overcode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OVERCODE_CONFIG && !Flag.OVERCODE_CONFIG_DIR && !Flag.OVERCODE_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OVERCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/overcode`
        ...
        if (Flag.OVERCODE_CONFIG) {
          yield* merge(Flag.OVERCODE_CONFIG, yield* loadFile(Flag.OVERCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.OVERCODE_CONFIG })
        }

        if (!Flag.OVERCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("overcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.OVERCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OVERCODE_CONFIG_DIR", { path: Flag.OVERCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".overcode") || dir === Flag.OVERCODE_CONFIG_DIR) {
            for (const file of ["overcode.json", "overcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.overcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OVERCODE_CONFIG_CONTENT) {
          const source = "OVERCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OVERCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from OVERCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OVERCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OVERCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OVERCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OVERCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("OVERCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.OVERCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OVERCODE_DISABLE_PRUNE) {
