...
export const DEPRECATED_PLUGIN_PACKAGES = ["overcode-openai-codex-auth", "overcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, overcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(overcodeVersion) || semver.major(overcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.overcode
  if (typeof range !== "string") return
  if (!semver.satisfies(overcodeVersion, range)) {
    throw new Error(`Plugin requires overcode ${range} but running ${overcodeVersion}`)
  }
}
