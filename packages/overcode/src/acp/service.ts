...
export class Service extends Context.Service<Service, Interface>()("@overcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `overcode auth login` in the terminal",
      name: "Login with overcode",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, overcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const overcodeProvider = providers[ProviderV2.ID.make("overcode")]
  const overcodeModel = overcodeProvider ? Provider.sort(Object.values(overcodeProvider.models))[0] : undefined
  if (overcodeProvider && overcodeModel) return { providerID: overcodeProvider.id, modelID: overcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Overcode service failure", service })
}
