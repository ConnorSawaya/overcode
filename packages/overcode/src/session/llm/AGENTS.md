...
`../llm.ts` is the overcode session LLM service. It owns overcode concerns: auth, config, model/provider resolution, plugins, permissions, telemetry headers, and runtime selection. It is the only file in this area that should know about the full session request shape.
...
- `native-request.ts` converts overcode's normalized session input into a native `@opencode-ai/llm` `LLMRequest`. It does not execute requests.
- `native-runtime.ts` is the opt-in native runtime adapter. It decides whether a selected model is supported, builds the native request, bridges overcode tools into native executable tools, and delegates transport to `LLMClient` / `RequestExecutor`.
...
src/session/
  ...
  llm/
    ...
    native-request.ts       overcode/AI SDK-shaped input -> @opencode-ai/llm LLMRequest
...
`native-runtime.ts` evaluates the gate and either bridges into `@opencode-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays overcode-owned in both branches; only request lowering and transport differ.

Safety boundary:

- AI SDK remains the default.
- `OVERCODE_EXPERIMENTAL_NATIVE_LLM=true` or the umbrella `OVERCODE_EXPERIMENTAL=true` opts in. Native is not a global replacement.
- Native execution currently supports OpenAI, overcode-managed OpenAI-compatible, and Anthropic API-key paths backed by `@ai-sdk/openai`, `@ai-sdk/openai-compatible`, or `@ai-sdk/anthropic` catalog entries.
