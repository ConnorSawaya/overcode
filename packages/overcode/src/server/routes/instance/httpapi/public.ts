...
import { OvercodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for overcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OvercodeHttpApi)
...
export const PublicApi = OvercodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "overcode",
    version: "1.0.0",
    description: "overcode api",
