...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("overcode-find")
  api.delete("overcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("overcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("overcode-find", new Highlight(...rest))
    return true
  }
