...
const parser = createMarkdownParser(async (code, language) => {
  ...
  return instance.codeToHtml(code, { lang: name as BundledLanguage, theme: "Overcode", tabindex: false })
})
...
async function highlight(request: Extract<MarkdownWorkerRequest, { type: "highlight" }>) {
  try {
    ...
    if (request.complete) {
      const result = instance.codeToTokens(request.text, { lang: language as BundledLanguage, theme: "Overcode" })
    ...
    const stream = reset
      ? {
          language,
          source: "",
          tokenizer: new ShikiStreamTokenizer({ highlighter: instance, lang: language, theme: "Overcode" }),
        }
      : previous
...
function getHighlighter() {
  return (highlighter ??= createHighlighter({ themes: [OvercodeTheme], langs: [] }))
}
