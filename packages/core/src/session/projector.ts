...
const promptQueueAddedType = EventV2.versionedType(
  SessionEvent.PromptQueueAdded.type,
  SessionEvent.PromptQueueAdded.durable!.version,
)
const promptQueueRemovedType = EventV2.versionedType(
  SessionEvent.PromptQueueRemoved.type,
  SessionEvent.PromptQueueRemoved.durable!.version,
)
...
const sessionMetadataEventTypes = new Set([
  EventV2.versionedType(SessionV1.Event.Created.type, SessionV1.Event.Created.durable!.version),
  EventV2.versionedType(SessionV1.Event.Updated.type, SessionV1.Event.Updated.durable!.version),
  EventV2.versionedType(SessionV1.Event.Deleted.type, SessionV1.Event.Deleted.durable!.version),
])
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        if (syncImported(event)) {
          const previous = yield* db
            ...
            .from(EventTable)
            .where(eq(EventTable.aggregate_id, event.data.sessionID))
            .all()
            .pipe(Effect.orDie)
          if (
            !importedEventWins(
              { time: event.data.info.time.updated, id: event.id },
              previous.filter((item) => sessionMetadataEventTypes.has(item.type)),
            )
          )
            return
        }
    ...
    yield* events.project(SessionV1.Event.Updated, (event) =>
      Effect.gen(function* () {
        ...
        if (
          !importedEventWins(
            { time: info.time.updated, id: event.id },
            previous.filter((item) => sessionMetadataEventTypes.has(item.type)),
          )
        )
          return
    ...
    yield* events.project(SessionEvent.PromptQueueAdded, (event) =>
      Effect.gen(function* () {
        const previous = yield* db
          .select({ id: EventTable.id, data: EventTable.data, type: EventTable.type })
          .from(EventTable)
          ...
          .all()
          .pipe(Effect.orDie)
        const queueEvents = previous.filter(
          (item) => item.type === promptQueueAddedType || item.type === promptQueueRemovedType,
        )
        if (!importedEventWins({ time: DateTime.toEpochMillis(event.data.timestamp), id: event.id }, queueEvents))
          return

        yield* db
          .insert(SessionPromptQueueTable)
          .values({
            id: event.data.messageID as typeof SessionPromptQueueTable.$inferInsert.id,
            session_id: event.data.sessionID,
            payload: event.data.input as Record<string, unknown>,
    ...
    yield* events.project(SessionEvent.PromptQueueRemoved, (event) =>
      Effect.gen(function* () {
        const previous = yield* db
          .select({ id: EventTable.id, data: EventTable.data, type: EventTable.type })
          .from(EventTable)
        ...
        yield* db
          ...
          .where(
            and(
              eq(SessionPromptQueueTable.session_id, event.data.sessionID),
              eq(SessionPromptQueueTable.id, event.data.messageID as typeof SessionPromptQueueTable.$inferSelect.id),
            ),
          )
          .run()
          .pipe(Effect.orDie)
