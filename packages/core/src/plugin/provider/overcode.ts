...
export const OvercodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "overcode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OvercodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("overcode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Overcode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("overcode", (integration) => {
        integration.name = "Overcode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "overcode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("overcode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("overcode")
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("overcode")),
