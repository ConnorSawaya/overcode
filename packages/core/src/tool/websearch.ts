...
export class ConfigService extends Context.Service<ConfigService, Config>()("@overcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OVERCODE_WEBSEARCH_PROVIDER === "exa" || process.env.OVERCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OVERCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("OVERCODE_EXPERIMENTAL") || truthy("OVERCODE_ENABLE_EXA") || truthy("OVERCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OVERCODE_ENABLE_PARALLEL") || truthy("OVERCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `overcode/${InstallationVersion}`,
