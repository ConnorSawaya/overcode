...
// etc.).
//
// The visual language mirrors the Overcode app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Overcode is now connected to ${escapeHtml(provider)}.` : "Overcode is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Overcode couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Overcode couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Overcode.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Overcode</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Overcode couldn't finish connecting to "+PROVIDER+"."):"Overcode couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Overcode."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Overcode is now connected to "+PROVIDER+"."):"Overcode is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Overcode wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
