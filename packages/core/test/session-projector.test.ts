...
describe("SessionProjector", () => {
  it.effect("does not let a late offline metadata edit overwrite a newer projection", () =>
    Effect.gen(function* () {
      ...
      const info = (title: string, updated: number) =>
      ...
      yield* events.publish(
        ...
        { sessionID, info: info("newer", 20) },
        { id: EventV2.ID.make("evt_sync_newer"), metadata: { syncImport: true } },
      )
      yield* events.publish(
        SessionV1.Event.Updated,
        { sessionID, info: info("older", 10) },
        { id: EventV2.ID.make("evt_sync_older"), metadata: { syncImport: true } },
      )

      expect(yield* db.select({ title: SessionTable.title }).from(SessionTable).get()).toEqual({ title: "newer" })
    }),
  )

  it.effect("keeps a newer session tombstone ahead of stale edits and recreates", () =>
    Effect.gen(function* () {
      const { db } = yield* Database.Service
      yield* db
        .insert(ProjectTable)
      ...
      yield* events.publish(
        SessionV1.Event.Updated,
        { sessionID, info: info("newer edit", 20) },
        { id: EventV2.ID.make("evt_sync_edit"), metadata: { syncImport: true } },
      )
      yield* events.publish(
        SessionV1.Event.Deleted,
        { sessionID, info: info("stale delete", 10) },
        { id: EventV2.ID.make("evt_sync_stale_delete"), metadata: { syncImport: true } },
      )
      expect(yield* db.select({ title: SessionTable.title }).from(SessionTable).get()).toEqual({ title: "newer edit" })

      yield* events.publish(
        SessionV1.Event.Deleted,
        { sessionID, info: info("new delete", 30) },
        { id: EventV2.ID.make("evt_sync_new_delete"), metadata: { syncImport: true } },
      )
      expect(yield* db.select({ id: SessionTable.id }).from(SessionTable).get()).toBeUndefined()

      yield* events.publish(
        SessionV1.Event.Created,
        { sessionID, info: info("stale recreate", 15) },
        { id: EventV2.ID.make("evt_sync_stale_recreate"), metadata: { syncImport: true } },
      )
      expect(yield* db.select({ id: SessionTable.id }).from(SessionTable).get()).toBeUndefined()
    }),
  )

  it.effect("projects legacy queued prompts with deterministic add/remove tombstones", () =>
    Effect.gen(function* () {
      const { db } = yield* Database.Service
      yield* db
        .insert(ProjectTable)
      ...
      yield* db
        ...
        .pipe(Effect.orDie)
      const events = yield* EventV2.Service
      const messageID = SessionV1.MessageID.make("msg_legacy_queue")
      const input = { sessionID, messageID, delivery: "queue", parts: [{ type: "text", text: "queued" }] }
      const at = (time: number) => DateTime.makeUnsafe(time)

      yield* events.publish(
        SessionEvent.PromptQueueAdded,
        { sessionID, messageID, timestamp: at(20), input },
        { id: EventV2.ID.make("evt_legacy_queue_add") },
      )
      expect(
        yield* db.select().from(SessionPromptQueueTable).where(eq(SessionPromptQueueTable.id, messageID)).get(),
      ).toMatchObject({ payload: input })

      yield* events.publish(
        SessionEvent.PromptQueueRemoved,
        { sessionID, messageID, timestamp: at(10), reason: "cancel" },
        { id: EventV2.ID.make("evt_legacy_queue_stale_remove") },
      )
      expect(
        yield* db.select().from(SessionPromptQueueTable).where(eq(SessionPromptQueueTable.id, messageID)).get(),
      ).toBeDefined()
      ...
      yield* events.publish(
        SessionEvent.PromptQueueRemoved,
        { sessionID, messageID, timestamp: at(30), reason: "cancel" },
        { id: EventV2.ID.make("evt_legacy_queue_remove") },
      )
      expect(
        yield* db.select().from(SessionPromptQueueTable).where(eq(SessionPromptQueueTable.id, messageID)).get(),
      ).toBeUndefined()
      ...
      yield* events.publish(
        SessionEvent.PromptQueueAdded,
        { sessionID, messageID, timestamp: at(25), input },
        { id: EventV2.ID.make("evt_legacy_queue_stale_add") },
      )
      expect(
        yield* db.select().from(SessionPromptQueueTable).where(eq(SessionPromptQueueTable.id, messageID)).get(),
      ).toBeUndefined()
