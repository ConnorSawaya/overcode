...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@overcode/acme")).toBe("@overcode/acme")
    expect(Npm.sanitize("@overcode/acme@1.0.0")).toBe("@overcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/overcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/overcode/acme.git" : spec
