...
This is the living design and status document for `@opencode-ai/codemode` and its existing V2 Overcode adapter.
...
`packages/overcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
