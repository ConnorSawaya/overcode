...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve overcode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "overcode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "overcode")).toBe("openai")
    expect(statProvider("big-pickle", "", "overcode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "overcode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep overcode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "overcode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "overcode"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep overcode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "overcode"), country: "US" })).toMatchObject([
