  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      if (input.delivery === "queue") {
        ...
        yield* SessionPromptQueue.enqueue(db, events, { ...input, messageID, delivery: "queue" })
        yield* sessions.touch(input.sessionID)
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          if (!lastUser || completed) {
            const promoted = yield* SessionPromptQueue.promote(db, events, { sessionID }, persistQueuedPrompt)
            if (promoted) {
              step = 0
              continue
            }
    ...
    const cancelPending = Effect.fn("SessionPrompt.cancelPending")(function* (input: {
      ...
      const removed = yield* SessionPromptQueue.cancel(db, events, input)
    ...
    const promotePending = Effect.fn("SessionPrompt.promotePending")(function* (input: {
      ...
      const promoted = yield* SessionPromptQueue.promote(db, events, input, persistQueuedPrompt)
