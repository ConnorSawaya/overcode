  ...
  return locks.withLock(input.sessionID)(
    Effect.gen(function* () {
      ...
      const removed = yield* db
        .delete(SessionPromptQueueTable)
        .where(and(eq(SessionPromptQueueTable.session_id, input.sessionID), eq(SessionPromptQueueTable.id, queued.id)))
        .returning({ id: SessionPromptQueueTable.id })
        .get()
        .pipe(Effect.orDie)
      if (!removed) return yield* Effect.die(new LifecycleConflict({ id: queued.id }))
      return true
    }),
  )
}
