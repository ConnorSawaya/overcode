  ...
  return {
    ...
    hooks: {
      "astro:build:done": async () => {
        ...
        const result = spawnSync(bunExecutable(), ["run", "../opencode/script/schema.ts", "./dist/config.json", "./dist/tui.json"], {
          shell: false,
          stdio: "inherit",
          windowsHide: true,
        })
        if (result.error || result.status !== 0) throw result.error ?? new Error("schema generation failed")
      },
    },
  }
}
...
function bunExecutable() {
  ...
  const candidates = [
    ...
    ...(process.env.Path ?? "")
      .split(delimiter)
      .filter(Boolean)
      .map((entry) => join(entry, "node_modules", "bun", "bin", "bun.exe")),
  ]
