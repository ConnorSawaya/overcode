...
With Overcode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Overcode uses our own `overcode` theme.
...
Overcode comes with several built-in themes.
...
- Want Overcode to match their terminal's appearance
...
Overcode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/overcode/themes/*.json` or `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.overcode/themes/*.json`
4. **Current working directory** - Defined in `./.overcode/themes/*.json`
