---
title: Plugins
description: Écrivez vos propres plugins pour étendre Overcode.
---

Les plugins vous permettent d'étendre Overcode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Overcode.
...
- `.overcode/plugins/` - Plugins au niveau du projet
- `~/.config/overcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/overcode/node_modules/`.
...
1. Configuration globale (`~/.config/overcode/overcode.json`)
2. Configuration du projet (`overcode.json`)
3. Répertoire global des plugins (`~/.config/overcode/plugins/`)
4. Répertoire des plugins du projet (`.overcode/plugins/`)
...
Overcode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK overcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre overcode.
...
:::note
Si vous utilisez l'application de bureau Overcode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez overcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à overcode :
...
L'assistant `tool` crée un outil personnalisé que overcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour overcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) pour plus de détails.
