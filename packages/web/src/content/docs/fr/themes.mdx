...
Avec Overcode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Overcode utilise notre propre thème `overcode`.
...
Overcode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Overcode corresponde à l'apparence de leur terminal
...
Overcode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/overcode/themes/*.json` ou `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.overcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.overcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/overcode/themes
vim ~/.config/overcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .overcode/themes
vim .overcode/themes/my-theme.json
```
