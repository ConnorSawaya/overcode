---
title: SDK
description: Client JS de type sécurisé pour le serveur overcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK overcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler overcode par programme.
...
Créez une instance de overcode :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `overcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${overcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de overcode, vous pouvez créer une instance client pour vous y connecter :
...
const textResults = await client.find.text({
  query: { pattern: "function.*overcode" },
})
