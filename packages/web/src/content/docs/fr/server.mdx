---
title: Serveur
description: Interagissez avec le serveur overcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `overcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client overcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `overcode.local` |
...
Définissez `OVERCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `overcode`, ou définissez `OVERCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `overcode serve` et à `overcode web`.
...
Lorsque vous exécutez `overcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur overcode pour interagir avec overcode par programmation.
:::

Cette architecture permet à overcode de prendre en charge plusieurs clients et vous permet d'interagir avec overcode par programme.

Vous pouvez exécuter `overcode serve` pour démarrer un serveur autonome. Si vous avez le
overcode TUI en cours d'exécution, `overcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Overcode [IDE](/docs/ide).
...
Le serveur overcode expose les API suivantes.
