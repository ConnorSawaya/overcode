---
title: Configuração
description: Usando a configuração JSON do overcode.
---

Você pode configurar o overcode usando um arquivo de configuração JSON.

---

## Formato

O overcode suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/overcode`) - padrões organizacionais
2. **Configuração global** (`~/.config/overcode/overcode.json`) - preferências do usuário
3. **Configuração personalizada** (`OVERCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`overcode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.overcode`** - agentes, comandos, plugins
6. **Configuração inline** (`OVERCODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.overcode` e `~/.config/overcode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/overcode`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do overcode em `~/.config/overcode/overcode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/overcode/tui.json`.
...
Adicione `overcode.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o overcode é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `OVERCODE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OVERCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.overcode`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.overcode`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`overcode.ai/config.json`**](https://opencode.ai/config.json).

A configuração do TUI usa [**`overcode.ai/tui.json`**](https://opencode.ai/tui.json).
...
Use `OVERCODE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `overcode.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `overcode serve` e `overcode web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor overcode.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `overcode.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do overcode através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o overcode tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/overcode/agents/` ou `.overcode/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o overcode recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`overcode run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/overcode/commands/` ou `.overcode/commands/`. [Saiba mais aqui](/docs/commands).
...
O overcode fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o overcode **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o overcode com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.overcode/plugins/` ou `~/.config/overcode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o overcode para usar apenas provedores específicos, em vez de desabilitá-los um a um.
