...
Você pode adicionar ferramentas externas ao overcode usando o _Model Context Protocol_, ou MCP. O overcode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do overcode](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/overcode`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O overcode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o overcode irá:
...
Se o servidor requer autenticação, o overcode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `overcode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o overcode armazenará os tokens de forma segura em `~/.local/share/overcode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no overcode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do overcode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o overcode à sua conta do Sentry.
