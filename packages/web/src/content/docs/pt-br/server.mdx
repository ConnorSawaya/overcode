---
title: Servidor
description: Interaja com o servidor overcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `overcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente overcode pode usar.
...
```bash
overcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `overcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
overcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OVERCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `overcode`, ou defina `OVERCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `overcode serve` quanto ao `overcode web`.

```bash
OVERCODE_SERVER_PASSWORD=your-password overcode serve
```
...
Quando você executa `overcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor overcode para interagir com o overcode programaticamente.
:::

Essa arquitetura permite que o overcode suporte múltiplos clientes e permite que você interaja com o overcode programaticamente.

Você pode executar `overcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do overcode em execução, `overcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do overcode [IDE](/docs/ide).
...
O servidor overcode expõe as seguintes APIs.
