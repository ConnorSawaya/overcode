...
Existem dois tipos de agentes no overcode; agentes primários e subagentes.
...
overcode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Overcode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Overcode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Overcode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `overcode.json`:
...
- Global: `~/.config/overcode/agents/`
- Por projeto: `.overcode/agents/`

```markdown title="~/.config/overcode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o overcode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do overcode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do overcode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Overcode Zen](/docs/zen), você usaria `overcode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="overcode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="overcode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `overcode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/ConnorSawaya/overcode).
:::
...
```markdown title="~/.config/overcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/overcode/agents/security-auditor.md"
---
