...
O Overcode Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Overcode. Você assina o Overcode Go e
...
O Overcode Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O Overcode Go funciona como qualquer outro provedor no Overcode.

1. Você entra no **<a href={console}>Overcode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Overcode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Overcode Go.
:::
...
O Overcode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Overcode
usa o formato `overcode-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`overcode-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Overcode também.
