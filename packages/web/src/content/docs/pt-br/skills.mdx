...
As habilidades do agente permitem que o overcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O overcode pesquisa nesses locais:

- Configuração do projeto: `.overcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/overcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o overcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.overcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.overcode/skills/git-release/SKILL.md` assim:
...
O overcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `overcode.json`:
...
**Para agentes embutidos** (em `overcode.json`):
