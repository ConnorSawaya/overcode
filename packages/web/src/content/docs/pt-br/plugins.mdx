---
title: Plugins
description: Escreva seus próprios plugins para estender o overcode.
---

Plugins permitem que você estenda o overcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do overcode.
...
- `.overcode/plugins/` - Plugins em nível de projeto
- `~/.config/overcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/overcode/node_modules/`.
...
1. Configuração global (`~/.config/overcode/overcode.json`)
2. Configuração do projeto (`overcode.json`)
3. Diretório de plugins global (`~/.config/overcode/plugins/`)
4. Diretório de plugins do projeto (`.overcode/plugins/`)
...
O overcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do overcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o overcode.
...
:::note
Se você estiver usando o aplicativo desktop overcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o overcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao overcode:
...
O helper `tool` cria uma ferramenta personalizada que o overcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o overcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) para detalhes.
