...
Com o overcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o overcode usa nosso próprio tema `overcode`.
...
O overcode vem com vários temas embutidos.
...
- Querem que o overcode corresponda à aparência do seu terminal
...
O overcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/overcode/themes/*.json` ou `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.overcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.overcode/themes/*.json`
