---
title: Server
description: Interact with overcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `overcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an overcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `overcode.local` |
...
Set `OVERCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `overcode`, or set `OVERCODE_SERVER_USERNAME` to override it. This applies to both `overcode serve` and `overcode web`.
...
When you run `overcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the overcode server to interact with overcode programmatically.
:::

This architecture lets overcode support multiple clients and allows you to interact with overcode programmatically.

You can run `overcode serve` to start a standalone server. If you have the
overcode TUI running, `overcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Overcode [IDE](/docs/ide) plugins.
...
The overcode server exposes the following APIs.
