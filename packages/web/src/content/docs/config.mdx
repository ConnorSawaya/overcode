---
title: Config
description: Using the Overcode JSON config.
---

You can configure Overcode using a JSON config file.

---

## Format

Overcode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="overcode.jsonc"
{
...
1. **Remote config** (from `.well-known/overcode`) - organizational defaults
2. **Global config** (`~/.config/overcode/overcode.json`) - user preferences
3. **Custom config** (`OVERCODE_CONFIG` env var) - custom overrides
4. **Project config** (`overcode.json` in project) - project-specific settings
5. **`.overcode` directories** - agents, commands, plugins
6. **Inline config** (`OVERCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/overcode/` on macOS) - admin-controlled
...
:::note
The `.overcode` and `~/.config/overcode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/overcode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/overcode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Overcode config in `~/.config/overcode/overcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/overcode/tui.json`.
...
Add `overcode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Overcode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `OVERCODE_CONFIG` environment variable.

```bash
export OVERCODE_CONFIG=/path/to/my/custom-config.json
overcode run "Hello world"
```
...
Specify a custom config directory using the `OVERCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.overcode` directory, and should
follow the same structure.

```bash
export OVERCODE_CONFIG_DIR=/path/to/my/config-directory
overcode run "Hello world"
```

The custom directory is loaded after the global config and `.overcode` directories, so it **can override** their settings.
...
Drop an `overcode.json` or `overcode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/overcode/` |
| Linux    | `/etc/overcode/`                         |
| Windows  | `%ProgramData%\overcode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Overcode reads managed preferences from the `ai.overcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Overcode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.overcode.managed.plist`
2. `/Library/Managed Preferences/ai.overcode.managed.plist`

The plist keys map directly to `overcode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.overcode.managed` PayloadType. The Overcode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.overcode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.overcode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.overcode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`overcode.ai/config.json`**](https://opencode.ai/config.json).

TUI config uses [**`overcode.ai/tui.json`**](https://opencode.ai/tui.json).
...
Use `OVERCODE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `overcode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `overcode serve` and `overcode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Overcode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `overcode.local`. Useful for running multiple instances on the same network.
...
If not specified, Overcode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Overcode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Overcode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Overcode actions on configured resources. Currently, policies can control which providers Overcode may use.
...
Overcode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Overcode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/overcode/agents/` or `.overcode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Overcode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`overcode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/overcode/commands/` or `.overcode/commands/`. [Learn more here](/docs/commands).
...
Overcode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Overcode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, overcode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Overcode with custom tools, hooks, and integrations.

Place plugin files in `.overcode/plugins/` or `~/.config/overcode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="overcode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["overcode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Overcode to only use specific providers rather than disabling them one by one.
