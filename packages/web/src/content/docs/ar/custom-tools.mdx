...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في overcode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في overcode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.overcode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/overcode/tools/`.
