...
مع Overcode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Overcode سمتنا `overcode`.
...
يأتي Overcode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Overcode مظهر terminal لديهم
...
يدعم Overcode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/overcode/themes/*.json` أو `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.overcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.overcode/themes/*.json`
