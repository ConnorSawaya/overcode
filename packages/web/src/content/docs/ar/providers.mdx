---
title: المزوّدون
description: استخدام أي موفّر LLM في Overcode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Overcode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Overcode.
...
في `~/.local/share/overcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Overcode.
...
Overcode Zen هي قائمة نماذج يوفّرها فريق Overcode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Overcode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Overcode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر overcode، ثم انتقل إلى [overcode.ai/auth](https://overcode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Overcode واستخدامه اختياري بالكامل.

---

## Overcode Go

Overcode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Overcode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Overcode.

1. شغّل الأمر `/connect` في TUI، واختر `Overcode Go`، ثم انتقل إلى [overcode.ai/auth](https://overcode.ai/zen).
...
لاستخدام Amazon Bedrock مع Overcode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل overcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `overcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Overcode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين overcode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل overcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات overcode.
...
تخزين الرمز في مساحة تخزين مصادقة overcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Overcode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Overcode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`overcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع overcode:
...
لاستخدام Google Vertex AI مع Overcode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر overcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [overcode-helicone-session](https://github.com/H2Shami/overcode-helicone-session) لتسجيل كل محادثة Overcode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Overcode مدرجة كجلسة مستقلة.
...
يمكنك ضبط overcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط overcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط overcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Overcode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/overcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Overcode:
...
3. انسخ مفتاح API لاستخدامه في Overcode.
...
6. **مهم**: قبل استخدام نماذج السحابة في Overcode، يجب سحب معلومات النموذج محليا:
...
Overcode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Overcode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Overcode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Overcode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Overcode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات overcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات overcode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات overcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع overcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `overcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Overcode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `overcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات overcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات overcode.
