...
يمكنك إضافة أدوات خارجية إلى Overcode باستخدام _Model Context Protocol_، أو MCP. يدعم Overcode خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Overcode](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/overcode` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Overcode تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Overcode بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Overcode المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `overcode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Overcode الرموز بشكل آمن في `~/.local/share/overcode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Overcode إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Overcode مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Overcode بحساب Sentry الخاص بك.
