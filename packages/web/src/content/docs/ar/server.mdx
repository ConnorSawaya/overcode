---
title: الخادم
description: تفاعل مع خادم overcode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `overcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل overcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `overcode.local` |
...
عيّن `OVERCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `overcode`، أو عيّن `OVERCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `overcode serve` و `overcode web`.
...
عند تشغيل `overcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم overcode للتفاعل مع overcode برمجيا.
:::

تتيح هذه البنية لـ overcode دعم عدة عملاء وتمكّنك من التفاعل مع overcode برمجيا.

يمكنك تشغيل `overcode serve` لبدء خادم مستقل. إذا كانت واجهة overcode في terminal (TUI)
قيد التشغيل، فسيبدأ `overcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Overcode لـ [IDE](/docs/ide).
...
يعرض خادم overcode واجهات API التالية.
