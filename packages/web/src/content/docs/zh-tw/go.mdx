...
Overcode Go 是一項**每月 $10 美元的低成本訂閱服務**，讓您能穩定使用受歡迎的開源寫程式模型。

Go 的運作方式與 Overcode 中的任何其他供應商相同。您訂閱 Overcode Go 並取得您的 API key。這是**完全可選的**，您不需要使用它也能使用 Overcode。
...
:::tip
我們測試了一組精選的模型和供應商，它們與 Overcode 搭配運作良好。
:::
...
Overcode Go 讓您可以用**每月 $10 美元**存取這些模型。

---

## 運作方式

Overcode Go 的運作方式與 Overcode 中的任何其他供應商相同。

1. 您登入 **<a href={console}>Overcode Zen</a>**，訂閱 Go，然後複製您的 API key。
2. 您在 TUI 中執行 `/connect` 命令，選擇 `Overcode Go`，然後貼上您的 API key。
3. 在 TUI 中執行 `/models` 即可查看透過 Go 可用的模型清單。

:::note
每個工作區只能有一位成員訂閱 Overcode Go。
:::
...
您的 Overcode 設定中的 [model id](/docs/config/#models) 使用 `overcode-go/<model-id>` 格式。例如，Kimi K3 在設定中應使用 `overcode-go/kimi-k3`。
