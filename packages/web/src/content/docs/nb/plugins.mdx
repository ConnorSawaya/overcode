...
description: Skriv dine egne programtillegg for å utvide Overcode.
---

Plugins lar deg utvide Overcode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Overcode.
...
- `.overcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/overcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/overcode/overcode.json`)
2. Prosjektkonfigurasjon (`overcode.json`)
3. Global plugin-katalog (`~/.config/overcode/plugins/`)
4. Prosjektpluginkatalog (`.overcode/plugins/`)
...
Overcode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Overcode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Overcode.
...
:::note
Hvis du bruker Overcode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Overcode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Overcode:
...
`tool`-hjelperen lager et tilpasset verktøy som Overcode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Overcode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) for detaljer.
