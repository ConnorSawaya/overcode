---
title: Tjener
description: Samhandle med Overcode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `overcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Overcode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `overcode.local` |
...
Sett `OVERCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `overcode` som standard, eller sett `OVERCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `overcode serve` og `overcode web`.
...
Når du kjører `overcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Overcode-serveren til å samhandle med Overcode programmatisk.
:::

Denne arkitekturen lar Overcode støtte flere klienter og lar deg samhandle med Overcode programmatisk.

Du kan kjøre `overcode serve` for å starte en frittstående server. Hvis du har
Overcode TUI kjører, vil `overcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Overcode [IDE](/docs/ide) plugins.
...
overcode-serveren viser følgende APIer.
