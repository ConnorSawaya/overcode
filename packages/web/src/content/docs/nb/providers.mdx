---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Overcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Overcode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Overcode-konfigurasjonen.
...
i `~/.local/share/overcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Overcode
konfig.
...
Overcode Zen er en liste over modeller levert av Overcode-teamet som har vært
testet og verifisert for å fungere godt med Overcode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Overcode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Overcode Zen` og gå til [overcode.ai/auth](https://overcode.ai/zen).
...
Det fungerer som alle andre leverandører i Overcode og er helt valgfritt å bruke.

---

## Overcode Go

Overcode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Overcode-teamet som har vært
testet og verifisert for å fungere godt med Overcode.

1. Kjør kommandoen `/connect` i TUI, velg `Overcode Go`, og gå til [overcode.ai/auth](https://overcode.ai/zen).
...
Slik bruker du Amazon Bedrock med Overcode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Overcode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `overcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i overcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere overcode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Overcode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Overcode-konfigurasjonen.
...
for å lagre token i Overcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Overcode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Overcode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`overcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Overcode:
...
Slik bruker du Google Vertex AI med Overcode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Overcode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Overcode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [overcode-helicone-session](https://github.com/H2Shami/overcode-helicone-session) plugin for automatisk å logge hver Overcode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Overcode-samtale oppført som en separat økt.
...
Du kan konfigurere Overcode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Overcode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Overcode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Overcode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/overcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Overcode:
...
3. Kopier API-nøkkelen for bruk i Overcode.
...
6. **Viktig**: Før du bruker skymodeller i Overcode, må du hente modellinformasjonen lokalt:
...
Overcode Zen er en liste over testede og verifiserte modeller levert av Overcode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Overcode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Overcode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Overcode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Overcode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Overcode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Overcode:
...
Du kan også tilpasse modeller gjennom Overcode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Overcode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `overcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Overcode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `overcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Overcode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Overcode-konfigurasjonen.
