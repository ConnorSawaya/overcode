...
Du kan legge til eksterne verktøy i Overcode ved å bruke _Model Context Protocol_ eller MCP. Overcode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="overcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/overcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Overcode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Overcode:
...
Hvis serveren krever autentisering, vil Overcode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `overcode mcp auth <server-name>`.
...
```bash
overcode mcp auth my-oauth-server
```
...
```bash
overcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Overcode lagre tokens sikkert i `~/.local/share/overcode/mcp-auth.json`.
...
overcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Overcode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Overcode-konfigurasjonen som alle andre verktøy.
...
```json title="overcode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
overcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Overcode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Overcode-samtale oppført som en separat økt.
