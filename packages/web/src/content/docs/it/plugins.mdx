---
title: Plugin
description: Scrivi plugin per estendere Overcode.
---

I plugin ti permettono di estendere Overcode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Overcode.
...
- `.overcode/plugins/` - plugin a livello progetto
- `~/.config/overcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/overcode/node_modules/`.
...
1. Config globale (`~/.config/overcode/overcode.json`)
2. Config di progetto (`overcode.json`)
3. Directory plugin globale (`~/.config/overcode/plugins/`)
4. Directory plugin di progetto (`.overcode/plugins/`)
...
Overcode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di overcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere overcode.
...
:::note
Se usi l'app desktop di Overcode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a overcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a overcode:
...
L'helper `tool` crea uno strumento personalizzato che overcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in overcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) per i dettagli.
