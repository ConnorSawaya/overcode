...
Con Overcode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Overcode usa il tema `overcode`.
...
Overcode include diversi temi integrati.
...
- Vuole che Overcode corrisponda all'aspetto del terminale
...
Overcode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/overcode/themes/*.json` o `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.overcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.overcode/themes/*.json`
