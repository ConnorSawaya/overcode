...
Per diagnosticare problemi con Overcode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\overcode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `overcode --log-level DEBUG`.

---

## Archiviazione

overcode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/overcode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\overcode`
...
Overcode Desktop esegue in background un server locale di Overcode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Overcode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/overcode/overcode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\overcode\overcode.jsonc`
...
Overcode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/overcode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\overcode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Overcode possa ricostruirla.

1. Chiudi completamente Overcode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/overcode`
- **Linux**: elimina `~/.cache/overcode` (oppure esegui `rm -rf ~/.cache/overcode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\overcode`

3. Riavvia Overcode Desktop.

---

### Risolvi problemi di connessione al server

Overcode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `overcode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OVERCODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OVERCODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Overcode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Overcode.

---

### Notifiche non visualizzate

Overcode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Overcode nelle impostazioni del sistema operativo, e
...
1. Chiudi Overcode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Overcode Desktop):

- `overcode.settings.dat` (desktop default server URL)
- `overcode.global.dat` e `overcode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Overcode:
...
Per capire a quali modelli hai accesso, esegui `overcode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\overcode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. overcode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\overcode`

2. Riavvia overcode per reinstallare i pacchetti provider piu recenti

Questo forzera overcode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
overcode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
