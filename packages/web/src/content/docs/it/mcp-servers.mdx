...
Puoi aggiungere strumenti esterni a Overcode usando il _Model Context Protocol_, o MCP. Overcode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/overcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Overcode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Overcode:
...
Se il server richiede autenticazione, Overcode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `overcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Overcode memorizzera' i token in modo sicuro in `~/.local/share/overcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Overcode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Overcode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Overcode al tuo account Sentry.
