---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Overcode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Overcode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Overcode avvia la TUI per la directory corrente.
...
Quando usi la TUI di Overcode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Overcode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Overcode. _Alias_: `/quit`, `/q`
...
Questo è separato da `overcode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Overcode attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `overcode.default`.
