...
description: Usa la configurazione JSON di Overcode.
---

Puoi configurare Overcode usando un file di configurazione JSON.

---

## Formato

Overcode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/overcode`) - default dell'organizzazione
2. **Config globale** (`~/.config/overcode/overcode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OVERCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`overcode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.overcode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `OVERCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.overcode` e `~/.config/overcode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/overcode`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Overcode in `~/.config/overcode/overcode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/overcode/tui.json`.
...
Aggiungi `overcode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Overcode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `OVERCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OVERCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.overcode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.overcode`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`overcode.ai/config.json`**](https://opencode.ai/config.json).

La configurazione TUI usa [**`overcode.ai/tui.json`**](https://opencode.ai/tui.json).
...
Usa `OVERCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `overcode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `overcode serve` e `overcode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Overcode.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `overcode.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Overcode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Overcode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/overcode/agents/` o `.overcode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Overcode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`overcode run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/overcode/commands/` o `.overcode/commands/`. [Scopri di piu](/docs/commands).
...
Overcode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, overcode **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Overcode con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.overcode/plugins/` o `~/.config/overcode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Overcode a usare solo alcuni provider invece di disabilitarli uno per uno.
