---
title: Server
description: Interagisci con il server overcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `overcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client overcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `overcode.local` |
...
Imposta `OVERCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `overcode`, oppure imposta `OVERCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `overcode serve` sia per `overcode web`.
...
Quando esegui `overcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server overcode per interagire con overcode in modo programmatico.
:::

Questa architettura permette a overcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `overcode serve` per avviare un server standalone. Se la TUI di overcode e gia in esecuzione, `overcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Overcode.
...
Il server overcode espone le seguenti API.
