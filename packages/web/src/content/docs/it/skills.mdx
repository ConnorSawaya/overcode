...
Le skill degli agenti permettono a Overcode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Overcode cerca in queste posizioni:

- Config di progetto: `.overcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/overcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Overcode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.overcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.overcode/skills/git-release/SKILL.md` cosi':
...
Overcode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `overcode.json`:
...
**Per agenti integrati** (in `overcode.json`):
