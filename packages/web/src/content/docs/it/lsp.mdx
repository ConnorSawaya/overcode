---
title: Server LSP
description: Overcode si integra con i tuoi server LSP.
---

Overcode può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Overcode include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `OVERCODE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e overcode apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di overcode.
