...
Overcode Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Overcode. Ti abboni a Overcode Go e
...
Overcode Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

Overcode Go funziona come qualsiasi altro provider in Overcode.

1. Accedi a **<a href={console}>Overcode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Overcode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Overcode Go.
:::
...
Overcode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Overcode config
utilizza il formato `overcode-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`overcode-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato Overcode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Overcode.
