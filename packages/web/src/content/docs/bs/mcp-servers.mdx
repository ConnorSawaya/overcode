...
Možete dodati vanjske alate u Overcode koristeći _Model Context Protocol_, ili MCP. Overcode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/overcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Overcode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Overcode će:
...
Ako server zahtijeva autentifikaciju, Overcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `overcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Overcode sigurno čuva tokene u `~/.local/share/overcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Overcode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Overcode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Overcode sa vašim Sentry nalogom.
