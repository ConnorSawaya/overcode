...
Agent skills omogucavaju Overcode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Overcode pretrazuje ove lokacije:

- Konfiguracija projekta: `.overcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/overcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Overcode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.overcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.overcode/skills/git-release/SKILL.md` ovako:
...
Overcode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `overcode.json`:
...
**Za ugradene agente** (u `overcode.json`):
