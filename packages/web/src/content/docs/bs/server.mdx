---
title: Server
description: Komunicirajte s overcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `overcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji overcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `overcode.local` |
...
Postavite `OVERCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `overcode`, ili postavite `OVERCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `overcode serve` i za `overcode web`.
...
Kada pokrenete `overcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite overcode server za programsku interakciju sa overcode-om.
:::

Ova arhitektura omogucava overcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `overcode serve` da startate standalone server. Ako je overcode TUI vec pokrenut, `overcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Overcode [IDE](/docs/ide) pluginovi.
...
overcode server izlaže sljedece API-je.
