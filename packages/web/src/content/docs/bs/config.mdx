---
title: Konfiguracija
description: Korištenje Overcode JSON konfiguracije.
---

Možete konfigurirati Overcode koristeći JSON konfiguracijski fajl.

---

## Format

Overcode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/overcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/overcode/overcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OVERCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`overcode.json` u projektu) - postavke specifične za projekat
5. **`.overcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OVERCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.overcode` i `~/.config/overcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/overcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Overcode konfiguraciju u `~/.config/overcode/overcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/overcode/tui.json`.
...
Dodajte `overcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Overcode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OVERCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OVERCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.overcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.overcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`overcode.ai/config.json`**](https://opencode.ai/config.json).

TUI konfiguracija koristi [**`overcode.ai/tui.json`**](https://opencode.ai/tui.json).
...
Koristite `OVERCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `overcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `overcode serve` i `overcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Overcode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `overcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Overcode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Overcode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/overcode/agents/` ili `.overcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Overcode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`overcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/overcode/commands/` ili `.overcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Overcode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Overcode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Overcode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.overcode/plugins/` ili `~/.config/overcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Overcode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
