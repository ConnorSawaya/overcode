---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Overcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Overcode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Overcode konfiguraciji.
...
u `~/.local/share/overcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Overcode
config.
...
Overcode Zen je lista modela koje je obezbedio Overcode tim koji su bili
testirano i potvrđeno da dobro radi sa Overcode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Overcode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite overcode i idite na [overcode.ai/auth](https://overcode.ai/auth).
...
Radi kao i svaki drugi provajder u Overcode i potpuno je opcionalan za korištenje.

---

## Overcode Go

Overcode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Overcode tim i koji su testirani i verificirani da dobro rade s Overcode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Overcode Go` i idite na [overcode.ai/auth](https://overcode.ai/zen).
...
Da biste koristili Amazon Bedrock s Overcode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete overcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `overcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Overcode.
:::
...
Možete konfigurirati overcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi overcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju overcode konfiguraciju.
...
da pohrani token u overcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Overcode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Overcode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`overcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `overcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s overcode:
...
Za korištenje Google Vertex AI s Overcode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće overcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem overcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [overcode-helicone-session](https://github.com/H2Shami/overcode-helicone-session) da automatski prijavite svaki Overcode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Overcode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati overcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati overcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati overcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Overcode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/overcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Overcode:
...
3. Kopirajte API ključ za korištenje u Overcode.
...
6. **Važno**: Prije upotrebe modela oblaka u Overcode, morate lokalno povući informacije o modelu:
...
Overcode Zen je lista testiranih i verifikovanih modela koju je obezbedio Overcode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Overcode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Overcode Zen**.

```txt
   /connect
```

3. Unesite svoj Overcode API ključ.
...
Također možete dodati dodatne modele putem vaše overcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše overcode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju overcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s overcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `overcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Overcode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `overcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Overcode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj overcode konfiguraciji.
