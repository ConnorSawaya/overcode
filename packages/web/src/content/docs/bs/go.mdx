...
Overcode Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Overcode-u. Pretplatite se na Overcode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Overcode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Overcode-om.
:::
...
Overcode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

Overcode Go radi kao bilo koji drugi provajder u Overcode-u.

1. Prijavite se na **<a href={console}>Overcode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Overcode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Overcode Go.
:::
...
Overcode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Overcode konfiguraciji
koristi format `overcode-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`overcode-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Overcode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Overcode koristite i bilo kojeg drugog provajdera.
