---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Overcode.
---

Dodaci vam omogućavaju da proširite Overcode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Overcode.
...
- `.overcode/plugins/` - Dodaci na nivou projekta
- `~/.config/overcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/overcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/overcode/overcode.json`)
2. Konfiguracija projekta (`overcode.json`)
3. Globalni direktorij dodataka (`~/.config/overcode/plugins/`)
4. Direktorij dodataka projekta (`.overcode/plugins/`)
...
Overcode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Overcode.
...
Spriječite overcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u overcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji overcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za overcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) za detalje.
