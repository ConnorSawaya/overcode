...
U Overcode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Overcode koristi nasu `overcode` temu.
...
Overcode dolazi sa vise ugradenih tema.
...
- Zele da Overcode odgovara izgledu njihovog terminala
...
Overcode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/overcode/themes/*.json` ili `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.overcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.overcode/themes/*.json`
