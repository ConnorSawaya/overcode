...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Overcode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Overcode.
...
:::note
Ovaj alat je dostupan samo kada je `OVERCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `OVERCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Overcode provajdera ili kada je varijabla `OVERCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Overcode:
