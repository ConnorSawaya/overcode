...
You can add external tools to Overcode using the _Model Context Protocol_, or MCP. Overcode supports both local and remote servers.
...
You can define MCP servers in your [Overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/overcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Overcode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Overcode will:
...
If the server requires authentication, Overcode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `overcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Overcode will store the tokens securely in `~/.local/share/overcode/mcp-auth.json`.
...
Your MCPs are available as tools in Overcode, alongside built-in tools. So you can manage them through the Overcode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Overcode to your Sentry account.
