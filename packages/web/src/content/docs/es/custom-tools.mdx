...
description: Cree herramientas que LLM pueda llamar en overcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de overcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.overcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/overcode/tools/`.
...
```ts title=".overcode/tools/database.ts" {1}
...
```ts title=".overcode/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".overcode/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".overcode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".overcode/tools/add.py"
import sys
...
```ts title=".overcode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".overcode/tools/add.py")
