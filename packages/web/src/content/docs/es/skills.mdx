...
Las habilidades del agente permiten a Overcode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Overcode busca estas ubicaciones:

- Configuración del proyecto: `.overcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/overcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Overcode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.overcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.overcode/skills/git-release/SKILL.md` así:
...
Overcode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `overcode.json`:
...
**Para agentes integrados** (en `overcode.json`):
