---
title: Complementos
description: Escriba sus propios complementos para extender Overcode.
---

Los complementos le permiten extender Overcode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Overcode.
...
- `.overcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/overcode/plugins/` - Complementos globales
...
```json title="overcode.json"
{
  ...
  "plugin": ["overcode-helicone-session", "overcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/overcode/node_modules/`.
...
1. Configuración global (`~/.config/overcode/overcode.json`)
2. Configuración del proyecto (`overcode.json`)
3. Directorio global de complementos (`~/.config/overcode/plugins/`)
4. Directorio de complementos del proyecto (`.overcode/plugins/`)
...
```json title=".overcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Overcode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".overcode/plugins/my-plugin.ts"
...
```js title=".overcode/plugins/example.js"
...
- `client`: Un cliente SDK overcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar overcode.
...
```js title=".overcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "overcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Overcode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que overcode lea archivos `.env`:

```javascript title=".overcode/plugins/env-protection.js"
...
```javascript title=".overcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a overcode:

```ts title=".overcode/plugins/custom-tools.ts"
...
El ayudante `tool` crea una herramienta personalizada a la que overcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para overcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) para obtener más detalles.
...
```ts title=".overcode/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".overcode/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
