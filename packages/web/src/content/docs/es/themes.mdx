...
Con Overcode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Overcode usa nuestro propio tema `overcode`.
...
Overcode viene con varios temas integrados.
...
- Quiere que Overcode coincida con la apariencia de su terminal
...
Overcode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/overcode/themes/*.json` o `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.overcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.overcode/themes/*.json`
