...
Overcode Go es una suscripción de bajo costo de **$10/mes** que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Overcode. Te suscribes a Overcode Go y
...
Overcode Go te da acceso a estos modelos por **$10/mes**.

---

## Cómo funciona

Overcode Go funciona como cualquier otro proveedor en Overcode.

1. Inicias sesión en **<a href={console}>Overcode Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Overcode Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Overcode Go.
:::
...
Overcode Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Overcode
usa el formato `overcode-go/<model-id>`. Por ejemplo, para Kimi K3, usarías
`overcode-go/kimi-k3` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Overcode.
