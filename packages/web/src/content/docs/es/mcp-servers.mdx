...
Puede agregar herramientas externas a Overcode utilizando el _Model Context Protocol_ o MCP. Overcode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/overcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Overcode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Overcode:
...
Si el servidor requiere autenticación, Overcode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `overcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Overcode almacenará los tokens de forma segura en `~/.local/share/overcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Overcode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Overcode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Overcode a su cuenta Sentry.
