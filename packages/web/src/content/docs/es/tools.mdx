...
Las herramientas permiten que LLM realice acciones en su código base. Overcode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Overcode.
...
:::note
Esta herramienta solo está disponible cuando `OVERCODE_EXPERIMENTAL_LSP_TOOL=true` (o `OVERCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Overcode o cuando la variable de entorno `OVERCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Overcode:

```bash
OVERCODE_ENABLE_EXA=1 overcode
```
