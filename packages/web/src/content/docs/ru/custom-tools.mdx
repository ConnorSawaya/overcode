...
description: Создавайте инструменты, которые LLM может вызывать в overcode.
---

Пользовательские инструменты — это создаваемые вами функции, которые LLM может вызывать во время разговоров. Они работают вместе со [встроенными инструментами ](/docs/tools) overcode, такими как `read`, `write` и `bash`.
...
- Локально, поместив их в каталог `.overcode/tools/` вашего проекта.
- Или глобально, поместив их в `~/.config/overcode/tools/`.
