...
Создайте `.overcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию overcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем overcode [config](/docs/config):
...
- Глобальный: `~/.config/overcode/commands/`
- Для каждого проекта: `.overcode/commands/`
...
overcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
