...
For at fejlfinde problemer med Overcode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\overcode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `overcode --log-level DEBUG`.

---

## Lagring

overcode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/overcode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\overcode`
...
Overcode Desktop kører en lokal Overcode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Overcode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/overcode/overcode.jsonc` (eller `~/.config/overcode/overcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/overcode/overcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\overcode\overcode.jsonc`
...
Overcode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/overcode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\overcode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Overcode kan genopbygge den.

1. Afslut Overcode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/overcode`
- **Linux**: slet `~/.cache/overcode` (eller kør `rm -rf ~/.cache/overcode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\overcode`

3. Start Overcode Desktop på ny.

---

### Løs problemer med serverforbindelse

Overcode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `overcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OVERCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OVERCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Overcode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Overcode.

---

### Meddelelser vises ikke

Overcode Desktop viser kun systemvarsler når:

- varsler er aktiveret for Overcode i dine OS-indstillinger, og
...
1. Afslut Overcode Desktop.
2. Find og slet disse filer (de findes i Overcode Desktop-appens datamappe):

- `overcode.settings.dat` (skrivebordsstandardserver URL)
- `overcode.global.dat` og `overcode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Overcode:
...
For at finde ud af hvilke modeller du har adgang til, kør `overcode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\overcode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. overcode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\overcode`

2. Start overcode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge overcode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
overcode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
