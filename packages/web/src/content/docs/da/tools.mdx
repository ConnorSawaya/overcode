...
Her er alle de innebygde verktøyene som er tilgængelige i Overcode.
...
:::note
Dette verktøyet er bare tilgængelig når `OVERCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `OVERCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Overcode-leverandøren eller når miljøvariabelen `OVERCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
