...
Agentferdigheter lar Overcode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Overcode søker etter disse stedene:

- Prosjektkonfigurasjon: `.overcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/overcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Overcode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.overcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.overcode/skills/git-release/SKILL.md` slik:
...
Overcode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `overcode.json`:
...
**For innebygde agenter** (i `overcode.json`):
