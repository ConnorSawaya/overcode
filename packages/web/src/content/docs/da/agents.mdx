...
Der er to typer agenter i Overcode; primære agenter og subagenter.
...
Overcode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Overcode leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Overcode leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Overcode's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `overcode.json`-konfigurationsfil:
...
- Globalt: `~/.config/overcode/agents/`
- Pr. projekt: `.overcode/agents/`
...
Hvis ingen temperatur er angivet, bruger Overcode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Overcode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Overcode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Overcode Zen](/docs/zen), vil du bruge `overcode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `overcode models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/ConnorSawaya/overcode).
:::
