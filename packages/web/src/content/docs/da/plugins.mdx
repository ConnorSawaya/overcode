---
title: Plugins
description: Skriv dine egne plugins for at udvide Overcode.
---

Plugins giver dig mulighed for at udvide Overcode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Overcodes standardadfærd.
...
- `.overcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/overcode/node_modules/`.
...
1. Global konfiguration (`~/.config/overcode/overcode.json`)
2. Projektkonfiguration (`overcode.json`)
3. Global plugin-mappe (`~/.config/overcode/plugins/`)
4. Projekt plugin bibliotek (`.overcode/plugins/`)
...
Overcode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En overcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide overcode.
...
:::note
Hvis du bruger Overcode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå overcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til overcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som overcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for overcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk) for detaljer.
