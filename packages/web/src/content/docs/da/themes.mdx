...
Med Overcode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Overcode vårt eget `overcode`-tema.
...
Overcode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/overcode/themes/*.json` eller `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.overcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.overcode/themes/*.json`
