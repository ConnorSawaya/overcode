---
title: Server
description: Interager med overcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `overcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en overcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `overcode.local` |
...
Indstil `OVERCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `overcode`, eller indstil `OVERCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `overcode serve` og `overcode web`.
...
Når du kører `overcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug overcode-serveren til at interagere med overcode programmatisk.
:::

Denne arkitektur lader overcode understøtte flere klienter og lader dig interagere med overcode programmatisk.

Du kan køre `overcode serve` for at starte en selvstændig server. Hvis du har
overcode TUI kørende, vil `overcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Overcode [IDE](/docs/ide) plugins.
...
Overcode-serveren viser følgende API'er.
