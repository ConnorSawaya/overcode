...
Overcode Go er et lavprisabonnement til **$10/måned**, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i Overcode. Du abonnerer på Overcode Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med Overcode.
:::
...
Overcode Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

Overcode Go fungerer som enhver anden udbyder i Overcode.

1. Du logger ind på **<a href={console}>Overcode Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `Overcode Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på Overcode Go.
:::
...
Overcode Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din Overcode config
bruger formatet `overcode-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `overcode-go/kimi-k3` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med Overcode.
