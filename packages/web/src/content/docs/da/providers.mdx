---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Overcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Overcode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Overcode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/overcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Overcode-konfigurationen.
...
Overcode Zen er en liste over modeller leveret af Overcode-teamet, som er blevet testet og verificeret til at fungere godt med Overcode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Overcode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg overcode og gå til [overcode.ai/auth](https://overcode.ai/auth).
...
Det fungerer som alle andre udbydere i Overcode og er helt valgfrit at bruge.

---

## Overcode Go

Overcode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Overcode-teamet, som er testet og verificeret til at fungere godt med Overcode.

1. Kør kommandoen `/connect` i TUI, vælg `Overcode Go`, og gå til [overcode.ai/auth](https://overcode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Overcode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører overcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `overcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Overcode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere overcode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at overcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem overcode-konfigurationen.
...
gemme tokenet i overcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Overcode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Overcode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`overcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Overcode:
...
Sådan bruger du Google Vertex AI med Overcode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører overcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem overcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [overcode-helicone-session](https://github.com/H2Shami/overcode-helicone-session) plugin for automatisk at logge hver Overcode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Overcode-samtale opført som en separat session.
...
Du kan konfigurere overcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere overcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere overcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Overcode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/overcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Overcode:
...
3. Kopier API-nøglen til brug i Overcode.
...
6. **Vigtigt**: Før du bruger skymodeller i Overcode, skal du hente modelinformationen lokalt:
...
Overcode Zen er en liste over testede og verificerede modeller leveret af Overcode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Overcode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Overcode Zen**.

   ```txt
   /connect
   ```

3. Indtast Overcode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem overcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem overcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Overcode:
...
Du kan også tilpasse modeller gennem overcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med overcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `overcode.json`-filen i projektkataloget:
...
`limit`-felterne lader Overcode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `overcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek overcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i overcode-konfigurationen.
