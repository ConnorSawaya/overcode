---
title: Server
description: Nutze den overcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `overcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein overcode-Client nutzen kann.
...
Setze `OVERCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `overcode`, kann aber mit `OVERCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `overcode serve` und `overcode web`.
...
Wenn du `overcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den overcode-Server, um overcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `overcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `overcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Overcode-[IDE](/docs/ide)-Plugins.
...
Der overcode-Server stellt folgende APIs bereit.
