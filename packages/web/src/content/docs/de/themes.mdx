...
Mit Overcode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Overcode das `overcode`-Theme.
...
Overcode liefert mehrere eingebaute Themes mit.
...
- Overcode optisch ans Terminal anpassen wollen
...
Overcode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/overcode/themes/*.json` oder `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.overcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.overcode/themes/*.json`
