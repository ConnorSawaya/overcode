---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Overcode zu erweitern.
---

Mit Plugins können Sie Overcode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Overcode zu ändern.
...
- `.overcode/plugins/` – Plugins auf Projektebene
- `~/.config/overcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/overcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/overcode/overcode.json`)
2. Projektkonfiguration (`overcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/overcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.overcode/plugins/`)
...
Overcode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Overcode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Overcode erweitern können.
...
:::note
Wenn Sie die Overcode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Overcode `.env`-Dateien liest:
...
Plugins können Overcode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Overcode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Overcode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk).
