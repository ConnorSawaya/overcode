---
title: LSP Server
description: Overcode lässt sich in Ihren LSP-Server integrieren.
---

Overcode kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

Overcode verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `OVERCODE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn LSP aktiviert ist und Overcode eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Overcode-Konfiguration aktivieren und anpassen.
