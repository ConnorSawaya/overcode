...
Overcode Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Overcode. Du abonnierst Overcode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Overcode zu verwenden.
...
Overcode Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

Overcode Go funktioniert wie jeder andere Provider in Overcode.

1. Du meldest dich bei **<a href={console}>Overcode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Overcode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Overcode Go abonnieren.
:::
...
Overcode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Overcode Config verwendet das Format `overcode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `overcode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Overcode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Overcode zu nutzen.
