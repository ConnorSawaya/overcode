...
Agent Skills erlauben Overcode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Overcode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Overcode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.overcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.overcode/skills/git-release/SKILL.md` zum Beispiel so:
...
Overcode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `overcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `overcode.json`):
