...
Wenn Overcode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\overcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `overcode --log-level DEBUG`.

---

## Speicher

overcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/overcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\overcode`
...
Overcode Desktop startet im Hintergrund einen lokalen Overcode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Overcode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/overcode/overcode.jsonc` (oder `~/.config/overcode/overcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/overcode/overcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\overcode\overcode.jsonc`
...
Overcode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/overcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\overcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Overcode ihn neu aufbauen kann.

1. Beende Overcode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/overcode`
- **Linux**: loesche `~/.cache/overcode` (oder fuehre aus: `rm -rf ~/.cache/overcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\overcode`

3. Starte Overcode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Overcode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `overcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OVERCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OVERCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Overcode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Overcode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Overcode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Overcode in den OS-Einstellungen aktiviert sind, und
...
1. Beende Overcode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Overcode Desktop):

- `overcode.settings.dat` (desktop default server URL)
- `overcode.global.dat` und `overcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Overcode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `overcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\overcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. overcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\overcode`

2. Starte overcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt overcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
overcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
