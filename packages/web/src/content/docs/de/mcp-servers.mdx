...
Sie können externe Tools zu Overcode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Overcode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/overcode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Overcode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Overcode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Overcode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `overcode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Overcode die Token sicher in `~/.local/share/overcode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Overcode verfügbar. Sie können sie also wie jedes andere Tool über die Overcode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Overcode mit Ihrem Sentry-Konto zu verbinden.
