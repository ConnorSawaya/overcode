---
title: Regeln
description: Lege eigene Anweisungen fuer overcode fest.
---

Du kannst overcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in overcode den Befehl `/init` aus.
...
So kann overcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht overcode sie zu erweitern.
...
overcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/overcode/AGENTS.md` speichern.
Diese gelten in allen overcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Overcode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/overcode/AGENTS.md` existiert)
...
Beim Start sucht overcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/overcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `overcode.json` oder in `~/.config/overcode/overcode.json` angeben.
...
overcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit overcode.json

Empfohlen ist das `instructions`-Feld in der `overcode.json`:
...
Du kannst overcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `overcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
