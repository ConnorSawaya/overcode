...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 Overcode には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
Overcode で利用可能なすべての組み込みツールを次に示します。
...
:::note
このツールは、`OVERCODE_EXPERIMENTAL_LSP_TOOL=true` (または `OVERCODE_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、Overcode プロバイダーを使用している場合、または `OVERCODE_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

Overcode の起動時に有効にするには:
