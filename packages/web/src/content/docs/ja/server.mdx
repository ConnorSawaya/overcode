---
title: サーバー
description: HTTP 経由で Overcode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`overcode serve` コマンドは、Overcode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `overcode.local` |
...
HTTP 基本認証でサーバーを保護するには、`OVERCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `overcode` になるか、`OVERCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`overcode serve` と `overcode web` の両方に当てはまります。
...
`overcode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Overcode サーバーを使用して、プログラムで Overcode と対話します。
:::

`overcode serve` を実行してスタンドアロンサーバーを起動できます。すでに Overcode TUI を実行している場合でも、`overcode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Overcode [IDE](/docs/ide) プラグインによって使用されます。
