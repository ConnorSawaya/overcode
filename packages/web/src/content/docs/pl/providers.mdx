---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w overcode.
---

import config from "../../../../config.mjs"
export const console = config.console

overcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji overcode.
...
w `~/.local/share/overcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim overcode
config.
...
Overcode Zen to lista modeli dostarczonych przez zespół overcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z overcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Overcode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Overcode Zen` i przejdź do [overcode.ai/auth](https://overcode.ai/zen).
...
Działa jak każdy inny dostawca w overcode i jest całkowicie opcjonalny w użyciu.

---

## Overcode Go

Overcode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół overcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z overcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Overcode Go` i przejdź do [overcode.ai/auth](https://overcode.ai/zen).
...
Aby używać Amazon Bedrock z overcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania overcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `overcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w overcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować overcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod overcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji overcode.
...
overcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować overcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`overcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem overcode:
...
Aby używać Google Vertex AI z overcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania overcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą overcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [overcode-helicone-session](https://github.com/H2Shami/overcode-helicone-session), aby automatycznie rejestrować każdą konwersację overcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja overcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować overcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować overcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować overcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem overcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/overcode).
:::
...
Aby korzystać z Ollama Cloud z overcode:
...
3. Skopiuj klucz API do użycia w overcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w overcode musisz pobrać informacje o modelu lokalnie:
...
Overcode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół overcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Overcode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Overcode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API overcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji overcode.
...
5. Możesz je także dostosować za pomocą konfiguracji overcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z overcode:
...
Możesz także dostosować modele za pomocą konfiguracji overcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z overcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `overcode.json` w katalogu projektu:
...
Pola `limit` pozwalają overcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `overcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację overcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji overcode.
