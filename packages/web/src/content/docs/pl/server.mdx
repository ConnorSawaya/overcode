---
title: Serwer
description: Interakcja z serwerem overcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `overcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient overcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `overcode.local` |
...
Ustaw `OVERCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `overcode` lub ustaw `OVERCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `overcode serve`, jak i `overcode web`.
...
Kiedy uruchomisz `overcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera overcode do programowej interakcji z overcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez overcode i programową interakcję z kodem otwartym.

Możesz uruchomić `overcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
overcode TUI działa, `overcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki overcode [IDE](/docs/ide).
...
Serwer overcode udostępnia następujące interfejsy API.
