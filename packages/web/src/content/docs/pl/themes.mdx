...
Dzięki overcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie overcode używa naszego własnego motywu `overcode`.
...
overcode ma kilka wbudowanych motywów.
...
- Chcą, aby Overcode pasował do wyglądu ich terminala
...
overcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/overcode/themes/*.json` lub `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.overcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.overcode/themes/*.json`
