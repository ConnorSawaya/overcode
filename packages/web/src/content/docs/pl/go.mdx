...
Overcode Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Overcode. Subskrybujesz Overcode Go i
...
Overcode Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

Overcode Go działa jak każdy inny dostawca w Overcode.

1. Logujesz się do **<a href={console}>Overcode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Overcode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Overcode Go.
:::
...
Overcode Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Overcode
używa formatu `overcode-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`overcode-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Overcode.
