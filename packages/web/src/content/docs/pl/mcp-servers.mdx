...
Możesz dodać dodatkowe informacje do overcode za pomocą _Model Context Protocol_, w skrócie MCP. overcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/overcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
overcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, overcode:
...
Jeśli serwer wymaga uwierzytelnienia, overcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `overcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji overcode bezpieczne przechowa tokeny w `~/.local/share/overcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w overcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie overcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć overcode z kontem Sentry.
