...
Aby debugować problemy z overcode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\overcode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `overcode --log-level DEBUG`.

---

## Przechowywanie danych

overcode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/overcode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\overcode`
...
overcode Desktop uruchamia w tle lokalny serwer overcode (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Overcode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/overcode/overcode.jsonc` (lub `~/.config/overcode/overcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/overcode/overcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\overcode\overcode.jsonc`
...
overcode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/overcode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\overcode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby overcode mógł ją odbudować.

1. Całkowicie zamknij overcode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/overcode`
- **Linux**: usuń `~/.cache/overcode` (lub uruchom `rm -rf ~/.cache/overcode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\overcode`

3. Uruchom ponownie overcode Desktop.

---

### Napraw problemy z połączeniem z serwerem

overcode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `overcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OVERCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OVERCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows overcode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami overcode.

---

### Brak powiadomień

overcode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla overcode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie overcode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji overcode Desktop):

- `overcode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `overcode.global.dat` i `overcode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z overcode:
...
3. Upewnij się, że masz najnowszą wersję z `overcode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `overcode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\overcode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. overcode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\overcode`

2. Uruchom ponownie kod overcode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to overcode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
overcode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
