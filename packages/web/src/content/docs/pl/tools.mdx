...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Overcode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Overcode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OVERCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `OVERCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Overcode lub gdy zmienna środowiskowa `OVERCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Overcode:
