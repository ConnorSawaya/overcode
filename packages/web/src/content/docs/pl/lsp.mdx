---
title: Serwery LSP
description: Overcode integruje się z serwerami LSP.
---

Overcode może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

Overcode posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `OVERCODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i Overcode otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji Overcode.
