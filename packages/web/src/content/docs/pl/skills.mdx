...
Umiejętności agenta pozwalają overcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
overcode przeszukuje te lokalizacje:

- Project config: `.overcode/skills/<name>/SKILL.md`
- Global config: `~/.config/overcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu overcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.overcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.overcode/skills/git-release/SKILL.md` w ten sposób:
...
overcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `overcode.json`:
...
**Dla agentów wbudowanych** (w `overcode.json`):
