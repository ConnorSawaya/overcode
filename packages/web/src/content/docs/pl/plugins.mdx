---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć overcode.
---

Wtyczki umożliwiające wyłączenie overcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania overcode.
...
- `.overcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/overcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/overcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/overcode/overcode.json`)
2. Konfiguracja projektu (`overcode.json`)
3. Globalny katalog wtyczek (`~/.config/overcode/plugins/`)
4. Katalog wtyczek projektu (`.overcode/plugins/`)
...
overcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z overcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do overcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej overcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj overcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do overcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod overcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/sdk).
