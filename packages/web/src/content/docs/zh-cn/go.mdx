...
Overcode Go 是一项**每月 10 美元的低成本订阅服务**，让你能够稳定地访问流行的开源编程模型。

Go 的工作方式与 Overcode 中的任何其他提供商（provider）一样。订阅 Overcode Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 Overcode 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 Overcode 配合良好的模型和提供商。
:::
...
Overcode Go 让你能够以**每月 10 美元**的价格访问这些模型。

---

## 工作原理

Overcode Go 的工作方式与 Overcode 中的其他提供商一样。

1. 登录 **<a href={console}>Overcode Zen</a>**，订阅 Go，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 `Overcode Go`，然后粘贴你的 API 密钥。
3. 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。

:::note
每个工作空间只能有一名成员订阅 Overcode Go。
:::
...
你的 Overcode 配置中的 [模型 ID](/docs/config/#models) 使用 `overcode-go/<model-id>` 格式。例如，对于 Kimi K3，你将在配置中使用 `overcode-go/kimi-k3`。
