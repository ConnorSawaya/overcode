---
title: TUI
description: Using the Overcode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Overcode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Overcode starts the TUI for the current directory.
...
When using the Overcode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Overcode. Allows you to select from available providers and add their API keys.
...
Exit Overcode. _Aliases_: `/quit`, `/q`
...
This is separate from `overcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Overcode waits after the leader key. Defaults to `2000`.
...
Use `OVERCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `overcode.default`.
