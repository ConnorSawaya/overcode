...
Create `.overcode/commands/test.md`:
...
You can add custom commands through the Overcode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Overcode [config](/docs/config):
...
overcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
