...
Tools allow the LLM to perform actions in your codebase. Overcode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Overcode.
...
:::note
This tool is only available when `OVERCODE_EXPERIMENTAL_LSP_TOOL=true` (or `OVERCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Overcode or Overcode Go provider, or when either the `OVERCODE_ENABLE_EXA` or `OVERCODE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Overcode:

```bash
OVERCODE_ENABLE_EXA=1 overcode
# or
OVERCODE_ENABLE_PARALLEL=1 overcode
```
