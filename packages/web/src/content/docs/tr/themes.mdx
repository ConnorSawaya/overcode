...
overcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak overcode kendi `overcode` temasını kullanır.
...
overcode birden fazla yerleşik temayla gelir.
...
- overcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
overcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/overcode/themes/*.json` veya `$XDG_CONFIG_HOME/overcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.overcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.overcode/themes/*.json`
