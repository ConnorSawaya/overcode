...
Ajan becerileri, overcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
overcode su konumlari tarar:

- Proje konfigurasyonu: `.overcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/overcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda overcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.overcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/overcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.overcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
overcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`overcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`overcode.json` icinde):
