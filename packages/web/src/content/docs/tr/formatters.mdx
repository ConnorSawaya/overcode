...
description: overcode dile özgü biçimlendiriciler kullanır.
---

overcode, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

overcode, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, overcode bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

overcode bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri overcode yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
