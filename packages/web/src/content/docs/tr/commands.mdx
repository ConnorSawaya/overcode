...
`.overcode/commands/test.md` oluştur:
...
overcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

overcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/overcode/commands/`
- Proje başına: `.overcode/commands/`
...
overcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
