...
description: Overcode, LSP sunucularınızla bütünleşir.
---

Overcode, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Overcode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OVERCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve overcode bir dosyayı açtığında:
...
LSP sunucularını, overcode yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
