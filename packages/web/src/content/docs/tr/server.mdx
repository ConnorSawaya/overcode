---
title: Sunucu
description: overcode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`overcode serve` komutu, overcode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `overcode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OVERCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `overcode` değeridir; değiştirmek isterseniz `OVERCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `overcode serve` hem de `overcode web` için geçerlidir.
...
`overcode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
overcode ile programatik etkileşim için overcode sunucusunu kullanın.
:::

Bu mimari, overcode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`overcode serve` ile bağımsız bir sunucu başlatabilirsiniz. overcode TUI açıksa, `overcode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum overcode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
overcode sunucusu aşağıdaki API'leri sunar.
