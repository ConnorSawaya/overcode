...
Overcode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, Overcode'daki diğer sağlayıcılar gibi çalışır. Overcode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Overcode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Overcode Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Overcode Go, Overcode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Overcode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Overcode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Overcode Go'ya abone olabilir.
:::
...
Overcode Go aşağıdaki limitleri içerir:
...
Overcode yapılandırmanızdaki [model id](/docs/config/#models) formatı `overcode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `overcode-go/kimi-k3` kullanmalısınız.
...
Overcode Go'yu şu amaçlarla oluşturduk:
...
4. Overcode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
