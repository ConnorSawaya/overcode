...
_Model Bağlam Protokolü_ veya MCP kullanarak overcode'a harici araçlar ekleyebilirsiniz. overcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [overcode Config](https://github.com/ConnorSawaya/overcode/tree/overcode-rebrand/packages/web/src/content/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/overcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
overcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde overcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa overcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `overcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından overcode, belirteçleri `~/.local/share/overcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra overcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi overcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve overcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
