  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://overcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://overcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://overcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://overcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://overcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://overcode.example.com"),
      ),
    ).toEqual({
