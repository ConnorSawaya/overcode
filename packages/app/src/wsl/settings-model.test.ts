...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Overcode needs attention", () => {
    ...
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/overcode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Overcode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        overcodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.overcode/bin/overcode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "overcode is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.primaryButton.action).toBe("install-overcode")
  })
