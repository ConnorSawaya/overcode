...
export function NewCodexSidebarHost() {
  ...
  const openQuickChat = async () => {
    ...
    if (!directory && conn && server.isLocal() && platform.quickChatDirectory) {
      try {
        directory = await platform.quickChatDirectory()
    ...
    await platform.openQuickChat?.({
      directory,
      sessionID: session?.id,
      serverKey: conn ? ServerConnection.key(conn) : undefined,
    })
  }
