...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("overcode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("overcode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("overcode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("overcode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("overcode://open-project")).toBeUndefined()
    expect(parseDeepLink("overcode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "overcode://open-project?directory=/a",
      "overcode://other?directory=/b",
      "overcode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("overcode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("overcode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("overcode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("overcode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "overcode://new-session?directory=/a",
      "overcode://open-project?directory=/b",
      "overcode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OVERCODE__: {
        deepLinks: ["overcode://open-project?directory=/a"],
      },
    } as unknown as Window & { __OVERCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["overcode://open-project?directory=/a"])
