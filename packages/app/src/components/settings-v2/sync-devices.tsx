  ...
  return (
    <>
      ...
      <div class="settings-v2-tab-body">
        <Show
          when={platform.platform === "desktop" && sync}
          fallback={<p>{language.t("settings.syncDevices.desktopOnly")}</p>}
        >
            ...
            <div class="settings-mobile-access-status" data-status={state.status}>
              ...
              <Show when={state.lastSynced}>
                <span class="settings-mobile-access-relay">
                  {language.t("settings.syncDevices.lastSynced", {
                    date: new Date(state.lastSynced!).toLocaleString(),
                  })}
                </span>
              </Show>
            </div>
                  ...
                  {(device) => (
                    <div class="settings-mobile-access-device">
                      <div>
                        ...
                        <span class="settings-mobile-access-device-type">
                          {device.deviceType ?? language.t("settings.syncDevices.deviceTypeUnknown")}
                        </span>
                        <span class="settings-mobile-access-device-meta">
                          {device.status === "online" ? `${language.t("settings.syncDevices.deviceOnline")} · ` : ""}
                          {language.t("settings.syncDevices.deviceLastSeen", {
          ...
          <Show when={state.missingProviders?.length}>
            <section class="settings-v2-section settings-sync-devices-credentials">
              <h3 class="settings-v2-section-title">{language.t("settings.syncDevices.credentialsTitle")}</h3>
              <p class="settings-sync-devices-copy">{language.t("settings.syncDevices.credentialsDescription")}</p>
              <div class="settings-mobile-access-devices">
                <For each={state.missingProviders}>
                  {(providerID) => (
                    <div class="settings-mobile-access-device">
                      <div>
                        <span class="settings-mobile-access-device-name">{providerID}</span>
                        <span class="settings-mobile-access-device-meta">
                          {language.t("settings.syncDevices.credentialsRequired")}
                        </span>
                      </div>
                    </div>
                  )}
