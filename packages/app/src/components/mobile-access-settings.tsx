    ...
    <Show when={access}>
      <section class="settings-v2-section settings-mobile-access" data-component="mobile-access-settings">
        ...
        <Show when={actionError()}>
          <p class="settings-mobile-access-error" role="alert">
            {language.t("settings.mobileAccess.error.actionFailed")}
          </p>
        </Show>
        <Show
          ...
          <Show when={(state.devices?.length ?? 0) > 0}>
            <div class="settings-mobile-access-devices">
              ...
              <For each={state.devices}>
                {(device) => (
                  <div class="settings-mobile-access-device" data-device-status={device.status ?? "offline"}>
                    <div>
                      <span class="settings-mobile-access-device-name">
                        <span class="settings-mobile-access-device-dot" aria-hidden="true" />
                        {device.name}
                      </span>
                      <span class="settings-mobile-access-device-type">
                        {device.deviceType ?? language.t("settings.mobileAccess.deviceTypeUnknown")}
                      </span>
                      <span class="settings-mobile-access-device-meta">
                        {device.status === "online" ? `${language.t("settings.mobileAccess.deviceOnline")} · ` : ""}
                        {language.t("settings.mobileAccess.deviceLastSeen", { date: new Date(device.lastSeen).toLocaleString() })}
                      </span>
                    </div>
