...
const directory = "C:/Overcode/FileBrowserSidebar"
...
async function setup(
  ...
  await mockOvercodeServer(page, {
    ...
    provider: {
      ...
      connected: ["overcode"],
      default: { providerID: "overcode", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "overcode.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "overcode.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "overcode.window.browser.dat:tabs",
