...
import { currentSession, mockOvercodeServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/Overcode/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#overcode-titlebar-right")
...
async function setup(page: Page, events?: () => EventPayload[]) {
  await mockOvercodeServer(page, {
    ...
    provider: {
      ...
      connected: ["overcode"],
      default: { providerID: "overcode", modelID: "claude-opus-4-6" },
    },
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "overcode", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "overcode.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("overcode.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
