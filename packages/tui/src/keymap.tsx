...
export const OVERCODE_BASE_MODE = "base"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OVERCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OVERCODE_MODE_KEY, stack.at(-1)?.mode ?? OVERCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OVERCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OVERCODE_MODE_KEY, undefined)
