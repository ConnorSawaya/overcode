...
export async function spawnWslSidecar(
  ...
  const overcode = await resolveWslOpencode(distro)
  if (!overcode) throw new Error(nativeT("desktop.wsl.error.overcodeNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "overcode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OVERCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OVERCODE_CLIENT=desktop",
    `export OVERCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export OVERCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(overcode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
