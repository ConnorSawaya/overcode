...
export function createQuickChatWindow(options: QuickChatOptions = {}, parent?: BrowserWindow | null) {
  if (quickChatWindow && !quickChatWindow.isDestroyed()) {
    // The singleton window can be opened from a different project or session.
    // Reload it with the new context instead of silently showing stale chat.
    loadWindow(quickChatWindow, `index.html?${quickChatQuery(options).toString()}`)
  ...
  const mode = tone()
  const query = quickChatQuery(options)
...
function quickChatQuery(options: QuickChatOptions) {
  ...
  if (options.serverKey?.trim()) query.set("server", options.serverKey.trim())
  return query
}
