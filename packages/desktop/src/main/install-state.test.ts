...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Overcode launch", () => {
    expect(hasExistingAppState([file("overcode.settings")])).toBe(true)
    expect(hasExistingAppState([file("overcode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("overcode")])).toBe(true)
  })
})
