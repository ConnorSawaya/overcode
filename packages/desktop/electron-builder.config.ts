...
const getBase = (appId: string): Configuration => ({
  artifactName: "overcode-desktop-${os}-${arch}.${ext}",
  directories: {
    output: "dist",
    buildResources: "resources",
  },
  // Linux launchers are .desktop files, so this is the desktop file name,
  // not just the app id. For prod, app id "ai.overcode.desktop" becomes
  // "ai.overcode.desktop.desktop".
  ...
  extraResources: [
    { from: "resources/computer", to: "computer", filter: ["*.exe"] },
    ...(channel === "dev"
      ? [
          {
            from: "resources/",
  ...
  switch (channel) {
    case "dev": {
      return {
        ...base,
        appId,
        productName: "Overcode Dev",
        deb: { fpm: [metainfoFpm(appId)] },
        rpm: { packageName: "overcode-dev", fpm: [metainfoFpm(appId)] },
      }
    }
    case "beta": {
      return {
