...
public class OvercodeSecureStoragePlugin extends Plugin {
    ...
    public void set(PluginCall call) {
        ...
        if (!isValidKey(key) || value == null || value.length() > MAX_VALUE_LENGTH) {
    ...
    private static boolean isValidKey(String key) {
        return key != null && key.matches("overcode\\.mobile\\.[A-Za-z0-9._-]{1,64}");
    }
    ...
    private String decrypt(String encoded) throws Exception {
        ...
        if (combined.length < 13 || combined.length > MAX_VALUE_LENGTH * 2) {
            throw new IllegalArgumentException("invalid secure value");
        }
        byte[] iv = new byte[12];
