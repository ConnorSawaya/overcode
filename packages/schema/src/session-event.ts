...
import { MessageID as LegacyMessageID } from "./v1/session"
...
// Compatibility events for the legacy prompt_async queue. The payload is kept
// opaque at the shared-schema boundary because the OpenCode-compatible server
// owns the richer prompt-input schema. The receiving projector still stores
// it verbatim and the server validates it before execution.
export const PromptQueueAdded = Event.define({
  type: "session.next.prompt.queue.added",
  ...options,
  schema: {
    ...Base,
...
export type PromptQueueAdded = typeof PromptQueueAdded.Type

export const PromptQueueRemoved = Event.define({
  type: "session.next.prompt.queue.removed",
  ...options,
  schema: {
    ...Base,
    messageID: LegacyMessageID,
    reason: Schema.Literals(["cancel", "promote"]),
  },
})
export type PromptQueueRemoved = typeof PromptQueueRemoved.Type
