...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "overcode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".overcode/glossary/zh-cn.md"
  if (locale === "zht") return ".overcode/glossary/zh-tw.md"
  return `.overcode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Overcode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Overcode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Overcode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Overcode runs for 'all' (default: 4)
      --model <provider/id>  Overcode model (default: overcode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Overcode
...
`)
  ...
  if (failed.length) console.error(`\nOvercode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.OVERCODE_DISABLE_PROJECT_CONFIG = "1"
  env.OVERCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["overcode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OVERCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["overcode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.OVERCODE_CONFIG
  delete env.OVERCODE_CONFIG_DIR
  delete env.OVERCODE_CONFIG_CONTENT
  delete env.OVERCODE_PERMISSION
  delete env.OVERCODE_AUTO_SHARE
  return env
}
