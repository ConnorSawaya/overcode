...
const model = "overcode/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with overcode...`)
  ...
  try {
    await $`overcode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  overcode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with overcode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with overcode...")
