...
Both `action` and `resource` use overcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.overcode` files is intentionally deferred until `.overcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether overcode permits an operation through its managed execution paths.
