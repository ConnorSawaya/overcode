...
Move the canonical Overcode terminal application from
`packages/overcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/overcode`,
`packages/cli`, or `@opencode-ai/core`.

The SDK is the TUI's Overcode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/overcode`.
- Do not replace private `packages/overcode` imports with `@opencode-ai/core`
...
- Overcode domain data displayed by the TUI
...
```text
packages/overcode/src/cli/cmd/tui
```

Its private dependency on `packages/overcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/overcode/src/*`.
...
- The package has no dependency on `overcode`, `@opencode-ai/cli`, or
  `@opencode-ai/core`.
...
route, and feature-plugin code no longer reads Overcode globals or process state;
...
Replace process-global Overcode state with resolved TUI inputs.
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/overcode` initially.
...
adapters rather than Overcode domain implementations.

Make the SDK the only Overcode domain boundary used by the TUI.
...
- Domain-facing TUI code imports Overcode data and operations only from
  `@opencode-ai/sdk`.
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/overcode`.
...
- The package has no imports from `packages/overcode`, `packages/cli`, or
  `@opencode-ai/core`.
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/overcode`.
- Keep the legacy embedded worker and server startup in `packages/overcode`.
...
- Remove obsolete `@tui/*` path mappings from `packages/overcode`.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Overcode domain state.
...
cd packages/overcode && bun typecheck
...
rg '@opencode-ai/core|packages/overcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/overcode/src packages/overcode/test
```
...
- Build the current-platform `packages/overcode` binary.
