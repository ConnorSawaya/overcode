...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an overcode storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/overcode` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No overcode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- Opencode-specific path/channel selection stays in `packages/overcode`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later overcode adoption PR.

The current `packages/overcode/src/storage/db.ts` has two non-obvious semantics that the overcode wrapper must preserve when it consumes this adapter:
...
The overcode wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving overcode to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.
...
4. Add `@opencode-ai/effect-drizzle-sqlite` as a dependency of `packages/overcode`.
5. Port `packages/overcode/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus overcode-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of overcode's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then overcode can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay overcode-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/overcode`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not overcode domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/overcode` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing overcode's current database runtime.
